function [b,Nr,Bstore,Cstore,tstore] = multispeciesbatch_variable_end(bo,Bt,Ct,s,P,m,p,plt,K,rc)

%Jaime Lopez 9/27/17
%This function computes one batch of m microbes competing for p substrates
%Monod growth, all growth parameters unity, adaptive timestepping. Here,
%the final simulation time can vary
...
%rc - remaining fraction of nutrient
...
while u == 0
    ...
    %COMPUTE TRUE dt
    true_gr = (1/6)*gr1 + (1/3)*gr2 + (1/3)*gr3 + (1/6)*gr4;
    true_con = (1/6)*con1 + (1/3)*con2 + (1/3)*con3 + (1/6)*con4;
    
    %ADJUST TIMESTEP FOR PRECISE BATCH END
    projected_C = sum(Ci - dt*true_con);
    if projected_C/Ct < rc
        current_dC = sum(dt*true_con);
        needed_dC = sum(Ci) - sum(rc*Ct);
        dt = dt*needed_dC/current_dC;
	tstore(tn) = tstore(tn-1) + dt; %Store time
    end
    ...
    if sum(C)/Ct <= rc %Check for depletion below determined threshold

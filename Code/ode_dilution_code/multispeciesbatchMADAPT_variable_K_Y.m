function [b,Nr,Bstore,Cstore,tstore] = multispeciesbatchMADAPT_variable_K_Y(bo,Bt,Ct,s,P,m,p,plt,K,Y)

%Jaime Lopez 9/27/17
%This function computes one batch of m microbes competing for p substrates
%Monod growth, all growth parameters unity, adaptive timestepping. K and
%yield can vary
...
gr = @(Bvar,Cvar) Bvar.*(s*(Y.*Cvar./(K + Cvar)));
...
if plt == 1
    ...
    for l=1:p
        hold on
        %plot(tstore,nu(l)*Cstore(l,:)./(K+Cstore(l,:)),'Color',colors2(l,:))
        plot(tstore,Cstore(l,:),'Color',colors2(l,:))
    end
    %title('Growth Function vs. Time')
    title('Concentration vs. Time')
    xlabel('Time')
    %ylabel('Y * Growth Function')
    ...
    figure
    if m > 1
        plot(tstore, sum(Cstore.*Y) + sum(Bstore))
    elseif m == 1
        plot(tstore, sum(Cstore.*Y) + Bstore)
    end

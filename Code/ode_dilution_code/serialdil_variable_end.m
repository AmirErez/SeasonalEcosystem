function [endb,endNr,bstore,Nrstore,end_ratio,end_t] = serialdil_variable_end(Bt,Ct,s,P,m,p,plt,bo,errtype,K,rc,max_batch)

%Jaime Lopez 9/28/17
%This function simulates serial dilutions where each dilution ends early
...
%rc - remaining fraction of nutrients
%max_batch - maximum number of batch to simulate for
...
while o == 0
    ...
    %Simulate batch
    [bstore(:,i+1),Nrstore(i,:)] = multispeciesbatch_variable_end(bstore(:,i),Bt,Ct,s,P,m,p,0,K,rc);
    ...
    if i > max_batch
        o = 1;
        disp('The model has reached the batch limit.')
        disp('The populations are')
        disp(bstore(:,i+1))
    end
end

end_ratio = bstore(:,i+1)./bstore(:,i);
end_t = i + 1;

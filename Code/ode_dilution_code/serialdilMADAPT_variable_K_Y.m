function [endb,endNr,bstore,Nrstore] = serialdilMADAPT_variable_K_Y(Bt,Ct,s,P,m,p,plt,bo,errtype,K,Y)

%Jaime Lopez 9/28/17
%This function simulates serial dilutions with variable K and yield
...
tol = 1e-7; %Relative absolute error tolerance for stopping transfers
extol = 1e-100; %Extinction tolerance for stopping transfers
...
while o == 0
    ...
    %Simulate batch
    [bstore(:,i+1),Nrstore(i,:)] = multispeciesbatchMADAPT_variable_K_Y(bstore(:,i),Bt,Ct,s,P,m,p,0,K,Y);
...
%     figure
%     if p == 2
%         plot((nu(1)*Nrstore(:,1)-nu(2)*Nrstore(:,2))./(nu(1)*Nrstore(:,1)+nu(2)*Nrstore(:,2)))
